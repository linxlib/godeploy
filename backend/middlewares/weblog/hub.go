...
func NewHub(logger logrus.FieldLogger, ts *TailService, wg *sync.WaitGroup) *Hub {
	return &Hub{
		log:         logger,
		workers:     ts,
		wg:          wg,
	...
	for {
		...
		case client := <-h.unregister:
			if _, ok := h.clients[client]; ok {
				h.log.Info("hub client unregister")
...
func (h *Hub) subscribe(channel string, client *Client) (string, bool) {
	...
	if !h.workers.WorkerExists(channel) {
		...
		if err != nil {
			h.log.Error(err, " ", MsgWorkerError)
...
func (h *Hub) send(client *Client, data []byte) bool {
	...
	default:
		h.log.Info("unsubscribeClient", " hub.send")
